...
var app = angular
    ...
    .service("quizService", function () {
        //service共享的是对象，不是普通的值
        //this.activate = false --> 这样的形式不会被共享！
        this.obj = {
            activate: false,
            activeQuestion: 0
        };
    ...
    .controller("ListController", ["$scope", "quizService", "dataService", "$log", function ($scope, quizService, dataService, $log) {
        $scope.quizService = quizService;
        $scope.name = "This is a turtle quiz";
        //for filter
        $scope.searchTurtle = "";

        $scope.turtlesData = dataService.dataObj.tutlesData;
        ...
        //$scope.activate = quizService.obj.activate;

        $scope.activateQuiz = function () {
            $scope.quizService.obj.activate = true;
            $scope.activate = $scope.quizService.obj.activate;
        }
    }])
    .controller("QuizController", ["$scope", "quizService", "dataService", "$log", function ($scope, quizService, dataService, $log) {
        $scope.quizService = quizService;
        $scope.activate = $scope.quizService.obj.activate;
        //we are not gonna to change the data 
        $scope.dataService = dataService;

        //For default question index
        $scope.activeQuestion = $scope.quizService.obj.activeQuestion;

        //count how many questions have been answered
        $scope.count = 0;

        $scope.changeIndex = function (index) {
            $scope.quizService.obj.activeQuestion = index;
            $scope.activeQuestion = $scope.quizService.obj.activeQuestion;
        }

        $scope.quizLength = dataService.dataObj.quizQuestions.length;

        $scope.questionAnswered = function () {
            $scope.quizService.obj.activeQuestion++;
            $scope.activeQuestion = $scope.quizService.obj.activeQuestion;

        }

        //controll the error message
        $scope.error = false;

        //we don't need this method again...
        //$scope.submitCheck = function () {
        //    for (var i = 0; i < dataService.dataObj.quizQuestions.length; i++) {
        //        if (dataService.dataObj.quizQuestions[i].selected === null) {
        //            $scope.error = true;
        //        }
        //    }
        //}

        //controll the 'submit' button


        $scope.selectedAnswer = function (index) {
            if ($scope.dataService.dataObj.quizQuestions[$scope.quizService.obj.activeQuestion].selected === null) {
                //count how many answers have been selected
                $scope.dataService.dataObj.check++;
            }

            $scope.dataService.dataObj.quizQuestions[$scope.quizService.obj.activeQuestion].selected = index;

        }

        //for finalise all the answers
        $scope.resultFlag = $scope.dataService.dataObj.resultFlag;

        $scope.finaliseAnswers = function () {

            for (var i = 0; i < $scope.quizLength; i++) {
                if ($scope.dataService.dataObj.correctAnswers[i] === $scope.dataService.dataObj.quizQuestions[i].selected) {
                    $scope.dataService.dataObj.quizQuestions[i].correct = true;
                    $scope.dataService.dataObj.correctNumbers++;
                    $scope.dataService.dataObj.correctAnswersArray.push(1);
                } else {
                    $scope.dataService.dataObj.quizQuestions[i].correct = false;
                    $scope.dataService.dataObj.correctAnswersArray.push(0);
                }
            }

            $scope.dataService.dataObj.resultFlag = true;
            $scope.resultFlag = $scope.dataService.dataObj.resultFlag;
            $scope.quizService.obj.activeQuestion = 0;
            $scope.dataService.dataObj.iconShow = true;


        }

        $scope.returnHomePage = function () {
            $scope.quizService.obj.activate = false;
            $scope.activate = $scope.quizService.obj.activate;
        }
    }])
    .controller("ResultController", ["$scope", "dataService", "quizService", "$log", function ($scope, dataService, quizService, $log) {
        $scope.dataService = dataService;
        $scope.quizService = quizService;
        $scope.activeQuestion = $scope.quizService.obj.activeQuestion;
        $scope.iconFlag = true;
        $scope.initIcon = function (flag) {
            $log.info("Icon Flag1: ", $scope.iconFlag);
            $scope.iconFlag = $scope.dataService.dataObj.iconShow;
            if (flag) {
                setTimeout(function () {
            ...
            $log.info("Icon Flag2: ", $scope.iconFlag);
        };
        ...
        // get answer class, using in ng-class
        $scope.getAnswerClass = function (index) {
            if (index === $scope.dataService.dataObj.correctAnswers[$scope.quizService.obj.activeQuestion]) {
                //then we can return the class 
                return "bg-success";
            } else if (index === $scope.dataService.dataObj.quizQuestions[$scope.quizService.obj.activeQuestion].selected) {
                return "bg-danger";
            }
        }
        ...
        $scope.calculatePerc = function () {
            return $scope.dataService.dataObj.correctNumbers / $scope.dataService.dataObj.quizQuestions.length;
        }

        //button for reset and go back to facts
        $scope.reset = function () {
            //active Question set to 0
            $scope.quizService.obj.activeQuestion = 0;
            $scope.quizService.obj.activate = false;
            $scope.dataService.dataObj.iconShow = true;
            $scope.dataService.dataObj.resultFlag = false;
            $scope.dataService.dataObj.check = 0;
            $scope.dataService.dataObj.correctAnswersArray = [];
            $scope.dataService.dataObj.correctNumbers = 0;
            for (var i = 0; i < dataService.dataObj.quizQuestions.length; i++) {
                var data = $scope.dataService.dataObj;
                data.quizQuestions[i].selected = null;
                data.quizQuestions[i].correct = null;
            }
        }
